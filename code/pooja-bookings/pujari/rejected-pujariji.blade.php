...
@section('title','Rejected PujariJi List')
@section('content')
    ...
    <div class="card">
        <div class="card-content">
            <div class="card-header d-flex justify-content-between">
                <h5 class="card-title p-3 pb-0 mb-1">Rejected PujariJi List</h5>
                            ...
                            @foreach ($data as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <span class="text-nowrap"><i class="fa fa-whatsapp text-success me-2 fs-5"></i> {{ $item->whatsapp_number }}</span>
                                    </td>
                                    <td>{{ $item->city }}</td>
                                    <td>{{ $item->state }}</td>
                                    @if(session('type')==base64_encode('1') || session('type')==base64_encode('master_admin'))
                                    <td>
                                        ...
                                        <a href="#" class="verify_reject" data-id="{{ $item->pujari_id }}"><span class="badge bg-danger">Delete Permanently</span></a>
                                    </td>
                                    @endif
                                </tr>
                            @endforeach
...
// Verify
 $('.verify_reject').click(function(){
    ...
    var ths = $(this);
    Swal.fire({
            title: "<br>Do you want to Verify/Delete Permanently Pujari Ji Request?<br><br>",
            showDenyButton: true,
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('delete_permanently') }}",
                data: {data:id},
                success: function (response) {
                    // toastr.success('Rejected Successfully');
                    Swal.fire("Pujariji Deleted Permanently", "", "danger");
                    ths.closest('tr').remove();
                }
            });
        }
