...
<!-- Add this line to your HTML file, preferably just before the closing </body> tag -->
                ...
                <tbody>
                    @foreach ($data as $value)
                        <tr>
                            ...
                            <td>{{ package_name($value->package_id)['eng_name'] }}</td>
                            <td>{{ get_pooja_name_by_package_id($value->package_id)['pooja_name'] }}</td>
                            <td>{{ $value->name }}</td>
                            <td>{{ $value->name_hindi }}</td>
                            <td>{{ $value->price }}</td>
                            @if(session('type')==base64_encode('1') || session('type')==base64_encode('master_admin'))
                            <td>
                                <a href="#" class="updateStatustoggle" data-url="{{ url('update-inclusion') }}" data-id="{{ $value->inclusion_id }}">
                                    @if($value->status=='0')
                                ...
                                <span class="mx-2">||</span>
                                <i class="fa fa-trash text-danger delete_this" data-id="{{ $value->inclusion_id }}" data-url="{{ url('delete-inclusion') }}"></i>
                            </td>
                            @endif
                        </tr>
                    @endforeach
